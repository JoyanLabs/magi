# Persistence Contract (shared across all PM skills)
...
|------|-----------|----------|---------------|
| `engram` | Engram (see `pm-engram-convention.md`) | Engram | Never |
| `openspec` | Filesystem (see `pm-openspec-convention.md`) | Filesystem | Yes |
...
- If mode is `openspec`, write files ONLY to the paths defined in `pm-openspec-convention.md`.
- NEVER force `.magi/` creation unless the orchestrator explicitly passed `openspec` mode.
